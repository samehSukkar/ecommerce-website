...
const ItemCard = ({item}) => {
  return (
        ...
        <Card style={cardStyles}>
                ...
                <Card.Body>
            <a href={`/products/${item.id}`} style={{textDecoration:'none', color:'black'}} >
                    ...
                    <div className="tags"  style={tagsStyles}>
                        {
                            item.tags.map((tag)=><Card.Link href={`/tags/${tag.id}`}key={tag.id}>#{tag.tagname}</Card.Link>)
                        }
                    </div>
                    <CartModal item={item} />
                </Card.Body>
...
const cardTitleStyles = {
    marginBottom: '5px',
    height: '3rem',
    //maxHeight: '3em', // twice the line height of the title
    overflow: 'hidden',
